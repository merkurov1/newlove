...
Причина: Это точно такая же проблема, как у нас была с jsonwebtoken. В вашем проекте есть файл (app/api/generate-post/route.ts), который пытается использовать библиотеку Google AI, но сама библиотека не установлена в проекте. Vercel не знает, откуда ее взять, и останавливает сборку.
Решение: Просто установить недостающий пакет.
Выполните в вашем терминале codespace эту команду:
npm install @google/generative-ai

Ваши следующие и финальные шаги:
 * Выполните команду в терминале, чтобы установить недостающую библиотеку Google:
   npm install @google/generative-ai

   Это действие изменит ваши файлы package.json и package-lock.json.
 * Замените все содержимое файла app/admin/actions.js на чистую версию, которую я предоставил выше.
 * Закоммитьте все изменения в Git. Это очень важно, так как Vercel будет смотреть на обновленный package.json.
   git add .
git commit -m "fix: Correct build errors and add missing dependency"
git push
