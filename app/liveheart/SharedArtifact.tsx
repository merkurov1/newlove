...
class Particle {
  x: number; y: number; z: number; vx: number; vy: number; size: number; baseSize: number; color: string; alpha: number;
  targetX?: number; targetY?: number; t: number; offset: number; speed: number; sparkleOffset: number;
...
export default function SharedArtifact({ dna }: { dna: HeartDNA }) {
  ...
  const phaseRef = useRef<'artifact'>('artifact');
    ...
    const repopulateParticles = (dnaObj: HeartDNA) => {
      ...
      for (let i = 0; i < dnaObj.particleCount; i++) {
        const p = new Particle(w, h);
        const t = (Math.PI * 2 * i) / dnaObj.particleCount;
        p.t = t;
        ...
        p.baseSize = dnaObj.particleSize + Math.random();
