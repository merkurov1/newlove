  ...
  return (
    <div className="min-h-screen bg-black text-white p-6">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-light mb-4">{data.title ?? dna?.name ?? 'LiveHeart'}</h1>
        <p className="mb-6 text-sm text-gray-300">Shareable LiveHeart artifact — created {new Date(data.created_at).toLocaleString()}</p>

        <div className="bg-white/5 p-4 rounded-md mb-6">
          <pre className="whitespace-pre-wrap text-xs">{JSON.stringify(dna, null, 2)}</pre>
        </div>
