    ...
    <div className="min-h-screen bg-black text-white font-sans pt-24 sm:pt-32 pb-24 px-4 sm:px-8">
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="flex justify-between items-center border-b border-zinc-800 pb-6 font-mono text-xs sm:text-sm text-zinc-400">
          <Link href="/lots" className="hover:text-white transition flex items-center gap-2">
            ← BACK TO ARCHIVE
          </Link>
          <a 
            href={lot.source_url} 
            ...
            {ai.tags && ai.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-6 border-t border-zinc-800">
                {ai.tags.map((tag: string, i: number) => (
                  <span key={i} className="text-xs font-mono bg-zinc-800/80 text-zinc-400 px-3 py-1 rounded-full border border-zinc-700/50">
                    #{tag}
                  </span>
                ))}
              </div>
