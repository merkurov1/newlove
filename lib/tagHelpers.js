...
export async function readArticleRelationsForTagStrict(supabase, tagId) {
  ...
  try {
    ...
    const q = (typeof from.select === 'function') ? from.select('*') : null;
    if (!q) return [];
    ...
    // Look for exact matches in common candidate columns (B, b, "B"), fallback to any field
    const candidates = ['B', 'b', 'A', 'a'];
    const matches = [];
    for (const row of rows) {
      ...
      for (const c of candidates) {
        if (Object.prototype.hasOwnProperty.call(row, c)) {
          const v = row[c];
          if (v && String(v) === String(tagId)) { matched = true; break; }
        }
      }
      if (!matched) {
        // Check any field as fallback (string contains)
        for (const k of Object.keys(row)) {
          const v = row[k];
          if (!v) continue;
          try {
            if (String(v) === String(tagId)) { matched = true; break; }
            if (typeof v === 'string' && v.includes && v.includes(String(tagId))) { matched = true; break; }
          } catch (e) { /* ignore */ }
        }
      }
