...
export async function readArticleRelationsForTagStrict(supabase, tagId) {
  ...
  try {
    // First attempt: try the REST endpoint (public/anon) if NEXT_PUBLIC_SUPABASE_URL is configured.
    // This often yields a plain JSON array and avoids some client-shape quirks.
    ...
    if (restUrl && restUrl.startsWith('http')) {
      try {
        const url = `${restUrl.replace(/\/$/, '')}/rest/v1/${encodeURIComponent(tbl)}?select=A,B&limit=2000`;
        // Use anon key if present to avoid exposing service role in non-explicit flows
        ...
        try {
          ...
          if (resp && resp.ok) {
            ...
            if (Array.isArray(json) && json.length > 0) {
              // Filter by B column (tag id) — support common shapes
              const matches = json.filter(r => r && (String(r.B) === String(tagId) || String(r.b) === String(tagId) || String(r.B).includes && String(r.B).includes(String(tagId))));
              if (matches.length > 0) return matches;
            }
          }
        } catch (e) {
          // fallthrough to client-based reader
        }
      } catch (e) {
        // ignore and continue to client path
      }
    }

    // Client-based fallback: select rows from the canonical table and filter in-memory
    const from = typeof supabase.from === 'function' ? supabase.from(tbl) : null;
    if (!from) return [];

    // Fetch a reasonable window of rows and filter in-memory by the B column (tag id)
    ...
    for (const row of rows) {
      ...
      if (Bval && String(Bval) === String(tagId)) {
        matches.push(row);
        continue;
      }
      // Fallback: scan any field for equality or contains
      let found = false;
      for (const k of Object.keys(row)) {
        const v = row[k];
        if (!v) continue;
        try {
          if (String(v) === String(tagId)) { found = true; break; }
          if (typeof v === 'string' && v.includes && v.includes(String(tagId))) { found = true; break; }
        } catch (e) { /* ignore */ }
      }
...
export async function getArticlesByTagStrict(supabase, tagSlugOrName, limit = 50) {
  ...
  try {
    ...
    // If client returned no rows, attempt REST endpoint fallback (anon key) as a tolerant read
    try {
