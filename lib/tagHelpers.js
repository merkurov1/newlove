...
function extractArticleIdFromRelRow(row) {
  ...
  const candidates = ['A', 'a', 'article_id', 'articleId', 'article', 'A_id', 'a_id', 'articleId1', 'article_id1'];
...
export async function getArticlesByTag(supabase, tagSlugOrName, limit = 50) {
  ...
  const ids = Array.from(new Set((rels || []).map(r => extractArticleIdFromRelRow(r)).filter(Boolean)));
