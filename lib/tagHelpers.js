...
export async function getArticlesByTagStrict(supabase, tagSlugOrName, limit = 50) {
  ...
  try {
    ...
    let rels = await readArticleRelationsForTag(supabase, tag.id) || [];
    let ids = Array.from(new Set((rels || []).map(r => extractArticleIdFromRelRow(r)).filter(Boolean))).map(String);

    // If we couldn't find ids via junction, attempt RPC to extract ids
    if (!ids || ids.length === 0) {
      try {
        const rpc = await supabase.rpc('get_articles_by_tag', { tag_slug: tagSlugOrName });
        const rpcData = (rpc && (rpc.data || rpc)) || [];
        if (Array.isArray(rpcData) && rpcData.length > 0) {
          ids = Array.from(new Set((rpcData || []).map(d => (d && (d.id || d.article_id || d.articleId)) || null).filter(Boolean))).map(String);
        }
      } catch (e) {
        // ignore rpc failure
      }
    ...
    // If still no ids, attempt to search article columns that may embed tag ids/slugs
    let arts = [];
    if (!ids || ids.length === 0) {
      const fallback = await findArticlesByArticleColumns(supabase, tag, limit) || [];
      if (Array.isArray(fallback) && fallback.length > 0) {
        arts = fallback;
      }
    }

    // If we have ids, query by ids strictly
    if (ids && ids.length > 0) {
      try {
        const res = await supabase.from('articles').select('id,title,slug,content,publishedAt,updatedAt,author').in('id', ids).limit(limit);
        arts = Array.isArray(res) ? res : (res && res.data ? res.data : []);
      } catch (e) {
        arts = [];
      }
    }
