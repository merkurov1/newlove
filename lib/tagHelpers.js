...
async function readArticleRelationsForTag(supabase, tagId) {
  ...
  for (const tbl of JUNCTION_CANDIDATES) {
    try {
      ...
      const tryCols = ['B', 'tag_id', 'tagId', 'b'];
...
export async function getArticlesByTag(supabase, tagSlugOrName, limit = 50) {
  ...
  const ids = Array.from(new Set((rels || []).map(r => r && (r.A || r.article_id || r.articleId || r.a || r.article || null)).filter(Boolean)));
      ...
      if (tag && tag.id) {
        try {
          ...
          if (rel && Array.isArray(rel)) {
            tagArticleIds = Array.from(new Set(rel.map(r => r && (r.A || r.article_id || r.articleId || r.a || r.article || null)).filter(Boolean)));
          }
        } catch (e) {
