...
export async function readArticleRelationsForTagStrict(supabase, tagId) {
  ...
  try {
    ...
    // Fallback: scan first N rows and filter in-memory for tagId present in any field
    try {
      const qAll = from.select('*');
      const qAllLimited = (typeof qAll.limit === 'function') ? qAll.limit(2000) : qAll;
      const resAll = await qAllLimited;
      const all = Array.isArray(resAll) ? resAll : (resAll && resAll.data ? resAll.data : []);
      if (Array.isArray(all) && all.length > 0) {
        const matches = all.filter(row => {
          ...
          for (const k of Object.keys(row)) {
            ...
            if (!v) continue;
            try {
              if (String(v) === String(tagId)) return true;
              // if array-like stored as string
              if (typeof v === 'string' && v.includes && v.includes(String(tagId))) return true;
              // if nested object
              if (typeof v === 'object') {
                for (const nk of Object.keys(v)) {
                  const nv = v[nk];
                  if (!nv) continue;
                  if (String(nv) === String(tagId)) return true;
                }
              }
            } catch (e) {
              continue;
