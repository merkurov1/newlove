...
export async function sendNewsletterToSubscriber(subscriber, letter, opts = {}) {
  ...
  // Log presence of key (masked) and from address for debugging
  try {
    const maskedKey = `${String(apiKey).slice(0,4)}...${String(apiKey).slice(-4)}`;
    console.info('Resend send: using API key', maskedKey, 'from', process.env.NOREPLY_EMAIL || 'noreply@merkurov.love');
  } catch (e) {}
    ...
    try {
      ...
      const providerResponse = { id: resp?.id || null, status: resp?.status || null, raw: resp };
