      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 transition-all duration-300">
        <div className="max-w-[1800px] mx-auto px-6 h-16 md:h-24 flex items-center justify-between">
          ...
          <Link href="/" className="z-50 group">
            <span className="font-sans font-bold text-lg md:text-xl tracking-[0.2em] uppercase text-zinc-900 group-hover:opacity-50 transition-opacity">
              Merkurov
            </span>
          </Link>
      ...
      >
          ...
          {navLinks.map((link) => (
            <Link
              key={link.name}
                  href={link.href}
                  className="text-3xl md:text-4xl font-serif font-medium text-zinc-900 hover:text-red-600 transition-colors"
            >
              {link.name}
            </Link>
          ))}
